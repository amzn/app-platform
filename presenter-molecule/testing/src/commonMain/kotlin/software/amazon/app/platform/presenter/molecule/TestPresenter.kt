 ...
 *
 * The used [CoroutineContext] for the presenter can be changed with [coroutineContext]. By default, a
 * [UnconfinedTestDispatcher] will be used, which means coroutines aren't confined to any thread and usually execute
 * immediately. This avoids unnecessary calls like `advanceUntilIdle`.
 *
...
public suspend fun <InputT : Any, ModelT : BaseModel> MoleculePresenter<InputT, ModelT>.test(
  ...
  testScope.moleculeScope(coroutineContext).launchMoleculePresenter(this, input).model.test(validate = validate)
}
