 ...
 *
 * This robot injects other dependencies from the object graph, e.g. this is helpful to query further data or change
 * behavior of classes. Robots are not exclusive to verifying UI and UI interactions.
 */
@Inject
...
class UserPageRobot(private val userManager: UserManager) : ComposeRobot() {
  ...
  /**
   * Verify that the user ID is displayed. The [userId] can be changed, but uses by default the ID of the logged in user
   * if present.
   */
