...
/**
 * Default implementation for [RendererFactory]. Implementations usually override [createRenderer] and [getRenderer] to
 * configure the [Renderer]s provided by the dependency graph further.
 */
public open class BaseRendererFactory(rootScopeProvider: RootScopeProvider) : RendererFactory {
  ...
  private val cacheKeys: Map<KClass<out BaseModel>, KClass<out Renderer<*>>> = rendererComponent.modelToRendererMapping
  ...
  override fun <T : BaseModel> createRenderer(modelType: KClass<out T>): Renderer<T> {
    ...
    return checkNotNull(renderers[modelType]?.invoke() as? Renderer<T>) { errorMessageForMissingRenderer(modelType) }
  }

  @Suppress("UNCHECKED_CAST")
  override fun <T : BaseModel> getRenderer(modelType: KClass<out T>, rendererId: Int): Renderer<T> {
    val rendererClass = checkNotNull(cacheKeys[modelType]) { errorMessageForMissingRenderer(modelType) }
  ...
  private inline fun <T : BaseModel> errorMessageForMissingRenderer(modelType: KClass<out T>): String {
